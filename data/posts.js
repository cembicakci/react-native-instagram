export default [
    {
        id: 1,
        description: `already missed old good days ⛱️🌞  #summer`,
        date: '2023-02-17 10:00:00',
        likes: 100,
        comments: 22,
        medias: [
            {
                src: 'https://images.unsplash.com/photo-1585541115062-e91a6580c409?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2787&q=80'
            }
        ],
        user: {
            name: 'cmbicakci',
            avatar: 'https://pbs.twimg.com/profile_images/1295728524490350592/li5FAiq7_400x400.jpg'
        }
    },
    {
        id: 2,
        description: `As Florida Artist RhondaK paintbrushes drying. I use colorful paint in my works on mermaids, manatees, crabs, palm trees and more. RhondaK.ar`,
        date: '2023-02-14 10:00:00',
        likes: 220,
        comments: 0,
        medias: [
            {
                src: 'https://images.unsplash.com/photo-1460661419201-fd4cecdf8a8b?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1180&q=80'
            }
        ],
        user: {
            name: 'RhondaK',
            avatar: 'https://images.unsplash.com/profile-1623102053386-c956ee5f0f0aimage?dpr=2&auto=format&fit=crop&w=150&h=150&q=60&crop=faces&bg=fff'
        }
    },
    {
        id: 3,
        description: `When you get older your wild heart will live for younger days. One day, you'll leave this world behind, so live a life you will remember`,
        date: '2023-02-13 10:00:00',
        likes: 120,
        comments: 34,
        medias: [
            {
                src: 'https://images.unsplash.com/photo-1554481923-a6918bd997bc?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1065&q=80'
            }
        ],
        user: {
            name: 'yannicladerach',
            avatar: 'https://images.unsplash.com/profile-fb-1509890684-fcf910cd9bc0.jpg?dpr=2&auto=format&fit=crop&w=150&h=150&q=60&crop=faces&bg=fff'
        }
    }
]